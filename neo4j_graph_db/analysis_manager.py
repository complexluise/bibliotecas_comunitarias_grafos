...
def main():
    ...
    coordinate_mappings = {
        ...
        "estado_digitalizacion": EstadoDigitalizacionCatalogoCoordinate(analyzer.driver, analyzer.df_encuestas),
        "porcentaje_coleccion": PorcentajeColeccionCatalogoCoordinate(analyzer.driver, analyzer.df_encuestas),
        "nivel_informacion": NivelInformacionCatalogoCoordinate(analyzer.driver, analyzer.df_encuestas),

        # Nivel de avance en la sistematización
        "sistemas_clasificacion": SistemasClasificacionCoordinate(analyzer.driver, analyzer.df_encuestas),
        "nivel_detalle": NivelDetalleOrganizacionColeccionCoordinate(analyzer.driver, analyzer.df_encuestas),
        "tiempo_busqueda": TiempoBusquedaLibroCoordinate(analyzer.driver, analyzer.df_encuestas),
        "sistema_registro": SistemaRegistroUsuariosCoordinate(analyzer.driver, analyzer.df_encuestas),
        "reglamento_servicios": ReglamentoServiciosCoordinate(analyzer.driver, analyzer.df_encuestas),
        "sistematizacion_prestamo": SistematizacionPrestamoExternoCoordinate(analyzer.driver, analyzer.df_encuestas),
        ...
        "percepcion_estado": PercepcionEstadoFisicoColeccionCoordinate(analyzer.driver, analyzer.df_encuestas),
        "enfoques_colecciones": EnfoquesColeccionesCoordinate(analyzer.driver, analyzer.df_encuestas),
        "actividades_mediacion": ActividadesMediacionColeccionCoordinate(analyzer.driver, analyzer.df_encuestas),
        "frecuencia_actividades": FrecuenciaActividadesMediacionCoordinate(analyzer.driver, analyzer.df_encuestas),
        "colecciones_especiales": ColeccionesEspecialesCoordinate(analyzer.driver, analyzer.df_encuestas),

        # Facilidad de Adopción del Catálogo en Koha
        "porcentaje_catalogada": PorcentajeColeccionCatalogadaCoordinate(analyzer.driver, analyzer.df_encuestas),
        "capacidad_tecnica": CapacidadTecnicaPersonalCoordinate(analyzer.driver, analyzer.df_encuestas),

        # Impacto de Adoptar Koha
        "nivel_impacto": NivelImpactoKohaCoordinate(analyzer.driver, analyzer.df_encuestas),
        "diversidad_servicios": DiversidadServiciosCoordinate(analyzer.driver),
        "sobrecarga_administrativa": SobrecargaAdministrativaCoordinate(analyzer.driver, analyzer.df_encuestas),
