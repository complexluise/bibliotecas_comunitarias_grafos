...
def crear_objetos_neo4j(row):
    ...
    tipos_coleccion = {
        f"Tiene{k.capitalize()}": v
        for k, v in row.items()
        if k.startswith("26_tipo_coleccion_")
    }
    coleccion.update(tipos_coleccion)

    servicios = {
        f"Ofrece{k.capitalize()}": v
        for k, v in row.items()
        if k.startswith("30_servicios_")
    }
    actividades = {
        f"Realiza{k.capitalize()}": v
        for k, v in row.items()
        if k.startswith("31_actividades_")
    }
    publico = {
        f"Atiende{k.capitalize()}": v
        for k, v in row.items()
        if k.startswith("34_poblacion_")
    }

    implementacion_koha = {
        "NivelInteres": safe_int(row.get("nivel_interes_catalogo")),
        "SobrecargaAdministrativa": safe_int(row.get("sobrecarga_admin_catalogo")),
        "BeneficiosOperacionales": safe_int(
            row.get("beneficios_operacionales_catalogo")
        ),
        "RiesgoResistenciaCambio": row.get("riesgo_resistencia_al_cambio", ""),
        "AccesibilidadCatalogoComunidad": row.get(
            "accesibilidad_catalogo_comunidad", ""
        ),
...
def cargar_datos_en_neo4j(uri, user, password, data):
    ...
    def create_nodes(tx, item):
        ...
        """,
            {
                "BibliotecaProps": {
                    k: v for k, v in item["Biblioteca"].items() if v is not None
                }
            },
        )
        ...
        """,
            {
                ...
                "CatalogoProps": {
                    k: v for k, v in item["Catalogo"].items() if v is not None
                },
            },
        )
        ...
        """,
            {
                ...
                "InfraestructuraProps": {
                    k: v for k, v in item["Infraestructura"].items() if v is not None
                },
            },
        )
        ...
        """,
            {
                "BibliotecaID": item["Biblioteca"]["ID"],
                "ColeccionProps": {
                    k: v for k, v in item["Coleccion"].items() if v is not None
                },
            },
        )
            ...
            """,
                {
                    "BibliotecaID": item["Biblioteca"]["ID"],
                    "EntityProps": {
                        k: v for k, v in item[entity_type].items() if v is not None
                    },
                },
            )
        ...
        """,
            {
                "BibliotecaID": item["Biblioteca"]["ID"],
                "ImplementacionKohaProps": {
                    k: v for k, v in item["ImplementacionKoha"].items() if v is not None
                },
            },
        )
